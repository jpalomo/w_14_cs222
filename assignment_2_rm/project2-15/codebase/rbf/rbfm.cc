...
RC RecordBasedFileManager::createFile(const string &fileName) {
	...
	if (returnValue == 0) {
		returnValue = pfm->createFile(("meta_" + fileName).c_str());
	}
 ...
 *
 * if length is shorter than 10 ( one short plus two unsigned which are need when this record becomes a tomb and need to hold new pageNum and slotNum), return 10
 */
